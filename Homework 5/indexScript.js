//#region Prerequisite variables
    ...
    const colors = [themeColors[0], "#ff0000", "#00ff00", "#0000ff", "#ff00ff"]; // The colors are white, red, green, blue, and ?something else?
...
//#region Helpers
    ...
    const getWorld = () => // don't worry, this being above arraybox does not mean that it will be called earlier than the arraybox assignment
    {
        return arraybox;
    }
...
//#region Assignment 1
    ...
    const createboxes = (count, canvasWidth, canvasHeight) =>
    {
        ...
        for(k = 0; k < population; k++)
        {
            ...
            outArray[k] = Box(currX, currY, boxSideLength, boxSideLength, colors[Math.floor(Math.random() * 4)], (Math.random() * 5) - (Math.random() * 5), (Math.random() * 5) - (Math.random() * 5), gravityF, airResistance, canvas.width, canvas.height, c)
        };
...
//#region Assignment 2
    ...
    let arraybox = createboxes(population, canvas.width, canvas.height);
    ...
    const redirect = () => 
    {
        ...
        {
            redirectVrgn = false;
            window.location.replace("https://MovsisyanMher.github.io/Pages/2.html")
            return true;
        }
    }
